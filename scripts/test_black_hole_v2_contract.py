...
release = json.loads(text('dist/v0.2.0-black-hole-v2-lab.1/release.json'))
...
require('RELEASE_MANIFEST' in loader and 'ab97b61ff3e21731cdb96bd0ed13d56641d08ddb' in loader, 'page-local loader pins the exact V2 staging manifest')
...
require(release['releasePurpose'] == 'unpublished-v2-staging', 'first V2 release is explicitly staging-only')
require(release['dataFoundation']['mode'] == 'temporary-pinned-seed', '.2 is recorded as temporary pinned staging input')
require('v0.1.0-black-hole-lab.2' in system['content']['url'], 'first staging release intentionally seeds from verified .2 content')
require(channel['channel'] == 'black-hole-v2-lab', 'V2 has its own isolated channel')
require(channel['currentRelease']['id'] == release['release'], 'V2 staging channel points at V2 staging release')
...
require('v0.1.0-black-hole-lab.2' not in builder, 'future V2 release architecture has no permanent .2 dependency')
